    ...
    task "3proxy" {
      ...
      service {
        ...
        tags = [
          ...
          # Traefik tag examples: https://doc.traefik.io/traefik/routing/providers/consul-catalog/
          "traefik.enable=true",
          "traefik.http.routers.3proxy.entrypoints=https",
          "traefik.http.routers.3proxy.rule=Host(`${ var.web_proxy_subdomain }.iddqd.uk`)",
          "traefik.http.routers.3proxy.tls.certResolver=lets-encrypt",
          "traefik.http.services.3proxy.loadbalancer.server.port=${NOMAD_HOST_PORT_http_proxy}",
        ]
